...
function create_complement_reversed_codons(data::CodonGraphData; show_debug::Bool=false)
    temp_codons = reverse_codon_set(complement_codon_set(data.codon_set, show_debug=show_debug), show_debug=show_debug)
...
function complement_codon(codon::AbstractString; show_debug::Bool=false)
    ...
    if length(codon) == 3
        show_debug && @debug "Original codon: $codon, -> complement codon: $(String([complement_base(c) for c in codon]))"
